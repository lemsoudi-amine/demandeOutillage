...
<div class="main-container">
	<div class="row gutter">
						...
						<table id="scrollTable" class="display table table-striped table-bordered no-margin" cellspacing="0" width="100%">
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center"><a id='{{ $porteur->id }}' class='btn btn-default btn-rounded icon-pencil modifyUser' href='{{ url('porteur/edit/'.$porteur->id) }}' title='Modification du porteur'></a>
										<form style="display: contents;" accept-charset="utf-8" id="form" name="form" method="post" action="{{ url('porteur/'.$porteur->id) }}" >
											@method('DELETE')
											@csrf
											<button id='{{ $porteur->id }}'type="submit" class='btn btn-default btn-rounded icon-delete_forever deletePorteur' style='padding-left: 10px;' title='Supprimer le porteur'></button>
									</form>
									</tr>																																												</td>
								@endforeach
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="panel">
				<div class="panel-heading">
					<h4>Création du porteur
						<h4>
				</div>
				<div class="panel-body">

						<form accept-charset="utf-8" id="form" name="form" method="post" action="{{ url('porteurs') }}">
								@if (count($errors))
						...
						<div class="form-group @if($errors->get('nameofporteur')) has-error @endif">
							<div class="row gutter">
								<div class="col-md-12">
									...
									<input id="nameofporteur" type="text" required class="form-control" placeholder="Nom du porteur" name="nameofporteur" value="{{ old('nameofporteur') }}">
								</div>
							</div>
						</div>
...
<script>
    new Noty({
        text:'<div class="icon i_tick notifico">le porteur a été ajouté avec succès</div>',
      	type: 'success',
		layout: 'center',
		timeout:2000,
       	animation: {
