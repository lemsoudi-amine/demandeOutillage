...
<div class="main-container">
	<div class="row gutter">
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="panel">
				<div class="panel-heading">
					<h4>Création de la section
						<h4>
				</div>
				<div class="panel-body">

						<form accept-charset="utf-8" id="form" name="form" method="post" action="{{ url('sections') }}">
								@if (count($errors))
						...
						<div class="form-group @if($errors->get('nameofsection')) has-error @endif">
							<div class="row gutter">
								<div class="col-md-12">
									...
									<input id="nameofsection" type="text" required class="form-control" placeholder="Nom de la section" name="nameofsection" value="{{ old('nameofsection') }}">
								</div>
							</div>
						</div>
						<div class="form-group @if($errors->get('num_section')) has-error @endif" >
								<div class="row gutter">
									<div class="col-md-12">
										...
										<input id="num_section" type="text" required class="form-control" placeholder="Numéro de la section" name="num_section" value="{{ old('num_section') }}">
									</div>
								</div>
							</div>
							<div class="form-group @if($errors->get('refeent_secteur')) has-error @endif" >
									<div class="row gutter">
										<div class="col-md-12">
											...
											<input id="refeent_secteur" type="text" required class="form-control" placeholder="Référent secteur" name="refeent_secteur" value="{{ old('refeent_secteur') }}">
										</div>
									</div>
								</div>
	...
	@include('../layouts.modalbilanimport')
</div>
...
@endif
@if (isset($statcode) && $statcode==400)
<script>
    new Noty({
        text:'<div class="icon i_tick notifico">Import effectué avec succès</div>',
      	type: 'success',
		layout: 'center',
		timeout:2000,
       	animation: {
...
@endif
@if (isset($statcode) && $statcode==500)
<script>
    new Noty({
        text:'<div class="icon i_tick notifico">Import des données non effectué. mauvais format du fichier</div>',
      	type: 'error',
		layout: 'center',
		timeout:2000,
       	animation: {
...
@endif
@if (isset($statcode) && $statcode==600)
<script>
    new Noty({
        text:'<div class="icon i_tick notifico">Import des données non effectué. aucun fichier choisi</div>',
      	type: 'error',
		layout: 'center',
		timeout:2000,
       	animation: {
